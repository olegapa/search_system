...
async def send_welcome(message: types.Message):  # type: ignore
    await message.answer('[ ]({}){}'.format(__PARAMS['START_GIF'],
                                            'Hi! My name is Patrick Boteman. I\'m sigma based cinemabot for '
                                            'searching films and series. '
                                            'There is an idea of me, '
                                            'some kind of abstraction. But there is no real me: only an entity, '
                                            'something illusory'),
...
async def bot_help(message: types.Message):  # type: ignore
    await message.answer('To find a film you just need to send me it name\n'
                         ...
                         f'Current is {__PARAMS["LANGUAGE"]}. Language choice '
...
async def film(message: types.Message):  # type: ignore
    async with aiohttp.ClientSession() as session:
        response = await session.get(url=f'https://imdb-api.com/{__PARAMS["LANGUAGE"]}/'
                                         f'API/Search/{__PARAMS["IMDB_API_KEY"]}/{message.text}')
        ...
        if response.status != 200:
            ...
            await message.answer("[ ]({}){}".format(info['image_url'],
                                                    info['short_info']),
                                 parse_mode='markdown', reply_markup=keyboard)
            await __DATABASE.append(message.from_user.id, info['full_info']['fullTitle'])
